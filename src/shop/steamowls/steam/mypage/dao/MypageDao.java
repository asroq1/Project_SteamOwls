...
public class MypageDao {
	...
	public BoardVo qModify(BoardVo boardVo) {
		...
		try {
			pstmt = con.prepareStatement("select member_sq, board_sq, board_subject, board_content, board_address, board_dttm"
					+ " from owls_board_tb where member_sq = ? and board_sq = ? and board_del_fl = 0");
			pstmt.setInt(1, boardVo.getMember_sq());
			pstmt.setInt(2, boardVo.getBoard_sq());
	...
	public BoardVo qModifyProc(BoardVo boardVo) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		BoardVo vo = null;
		try {
			pstmt = con.prepareStatement(
					"update owls_board_tb set (board_subject, board_content) values(?, ?)");
